...
class GameController extends ChangeNotifier {
  ...
  /// Pausa la partida y el temporizador (p. ej. al abrir Leaderboard o Ajustes).
  void pauseGame() {
    final game = _game;
    ...
    try {
      ...
      // Ignorar si la partida ya no admite pausa.
    }
  }

  /// Reanuda la partida y el temporizador al volver a la pantalla de juego.
  void resumeGame() {
    final game = _game;
    if (game == null || game.status != GameStatus.paused) return;
    ...
    try {
      ...
      // Ignorar si la partida ya no está pausada.
    }
  }
  ...
  Future<void> onCellTapped(Position position) async {
    ...
    await _audioService.ensureAudioUnlocked();
  ...
  Future<void> _playLossAudio(Game game) async {
    if (game.lossMessage == GameLossMessage.timeExceeded) {
      ...
      await _audioService.playNoMovementsLeft();
    }
  }
